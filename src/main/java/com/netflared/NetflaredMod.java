...
public class NetflaredMod {
    ...
    @SubscribeEvent public static void onClientTick(ClientTickEvent e){if(e.phase != net.neoforged.neoforge.event.TickEvent.Phase.END)return;Minecraft c=Minecraft.getInstance();Translations.refreshIfChanged();if(openTunnelKey==null)return;while(openTunnelKey.consumeClick()){Screen s=c.screen;if(s instanceof TitleScreen||s instanceof JoinMultiplayerScreen)c.setScreen(new NetflaredSettingsScreen(s));}}
