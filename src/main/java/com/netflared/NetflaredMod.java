...
public class NetflaredMod {
    ...
    @SubscribeEvent public static void onClientTick(ClientTickEvent e){Minecraft c=Minecraft.getInstance();if(e.phase != net.neoforged.neoforge.event.tick.ServerTickEvent.Phase.END)return;Translations.refreshIfChanged();if(openTunnelKey==nullreturn;while(openTunnelKey.consumeClick()){Screen s=c.screen;if(s instanceof TitleScreen||s instanceof JoinMultiplayerScreen)c.setScreen(new NetflaredSettingsScreen(s));}}
